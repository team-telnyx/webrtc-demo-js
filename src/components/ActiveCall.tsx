...
const ActiveCall = ({ call, title = 'Active Call' }: Props) => {
  ...
  const defaultAudioInputId = useMemo(() => {
    if (!audioInDevices.length || !call.localStream) {
      return '';
    }
    ...
    return defaultDevice?.deviceId || audioInDevices[0].deviceId;
  }, [call.localStream, audioInDevices]);

  const effectiveSelectedAudioInputId =
    selectedAudioInputId || defaultAudioInputId;

  const switchAudioInput = async () => {
    try {
      await call.setAudioInDevice(
        effectiveSelectedAudioInputId,
  ...
  const disconnectSocket = () => {
    ...
    if (typeof socketDisconnect !== 'function') {
      console.error(
        'Current TelnyxRTC client does not expose socketDisconnect',
      );
      return;
    }
        ...
        {audioInDevices.length > 0 && (
          <div className="space-y-3 rounded-lg border p-4">
            ...
            <div className="flex items-center justify-between gap-3">
              <Select
                value={effectiveSelectedAudioInputId || undefined}
              ...
              <Button size="sm" variant={'default'} onClick={switchAudioInput}>
                Apply
              </Button>
            </div>
          </div>
